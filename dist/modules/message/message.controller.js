...
exports.pinCourseMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?.id;
    const courseId = req.params.courseId;
    ...
    if (!messageId) {
        throw new errors_1.AppError(http_status_codes_1.StatusCodes.BAD_REQUEST, "Message id is required");
    }
    const membership = await (0, auth_1.ensureCourseMembership)(courseId, userId);
    if (!membership.isLecturer) {
        throw new errors_1.AppError(http_status_codes_1.StatusCodes.FORBIDDEN, "Only the course lecturer can pin messages");
    }
    const message = await (0, message_service_1.pinMessage)(courseId, messageId, userId);
    (0, chat_handlers_1.broadcastCourseMessagePinned)(getSocketInstance(req), message);
    res.status(http_status_codes_1.StatusCodes.OK).json(message);
});
// DELETE /api/courses/:courseId/messages/:messageId/pin
exports.unpinCourseMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?.id;
    const courseId = req.params.courseId;
    const messageId = req.params.messageId;
    if (!userId) {
    ...
    const membership = await (0, auth_1.ensureCourseMembership)(courseId, userId);
    if (!membership.isLecturer) {
        throw new errors_1.AppError(http_status_codes_1.StatusCodes.FORBIDDEN, "Only the course lecturer can unpin messages");
    }
    const message = await (0, message_service_1.unpinMessage)(courseId, messageId);
    (0, chat_handlers_1.broadcastCourseMessageUnpinned)(getSocketInstance(req), message);
    res.status(http_status_codes_1.StatusCodes.OK).json(message);
});
